...
class ServerError(InvalidUsage):
    status_code = 500
...
def memoised_query(method, query, excluding, exactly_one, callback=None):
    geocoding_pool = get_geocoding_pool(get_config(excluding, exactly_one))
    try:
        result = getattr(geocoding_pool, method)(query)
    except Exception as exc:
        print(exc)
        raise ServerError('Problem with query')
...
def forward(exactly_one=False):
    ...
    excluding = tuple(request.args.getlist('excluding'))
    exactly_one = bool(int(request.args.get('one', exactly_one)))
    if not address:
        raise InvalidUsage('address parameter must be specified',
                           status_code=400)
    return memoised_query('geocode', address, excluding, exactly_one,
                          lambda x: jsonify(location_summary(x)))
...
def forward_cluster(exactly_one=False):
    ...
    if not address:
        raise InvalidUsage('address parameter must be specified',
                           status_code=400)
    return memoised_query('geocode', address, excluding, exactly_one,
                          lambda x: jsonify(location_cluster_summary(x)))
...
def reverse(exactly_one=False):
    ...
    if None in (lat, lon):
        raise InvalidUsage('lat and lon parameters must be specified',
                           status_code=400)
    try:
        address = memoised_query('reverse', (float(lat), float(lon)),
                                 excluding, exactly_one, reverse_summary)
